...
def tmp_obs_path(tmp_path: Path) -> Path:
    return corpus_helpers.ref(tmp_path, persistence.OBSERVATIONS_SUBDIR, 2026)


def test_append_new_rows_written(tmp_obs_path):
    row = make_corpus_row()
    result = persistence.append_observation_rows(tmp_obs_path.base, tmp_obs_path.season, [row])
...
def test_append_same_row_twice_deduped_no_overwrite(tmp_obs_path):
    row = make_corpus_row()
    persistence.append_observation_rows(tmp_obs_path.base, tmp_obs_path.season, [row])
    result2 = persistence.append_observation_rows(tmp_obs_path.base, tmp_obs_path.season, [row])
    assert result2.written == 0
    assert result2.skipped_duplicate == 1
    rows = persistence.read_observation_rows(tmp_obs_path.sources)
    assert len(rows) == 1  # never a second copy


def test_within_batch_duplicates_also_deduped(tmp_obs_path):
    row = make_corpus_row()
    result = persistence.append_observation_rows(tmp_obs_path.base, tmp_obs_path.season, [row, row, row])
    assert result.written == 1
...
def test_different_keys_both_appended(tmp_obs_path):
    ...
    persistence.append_observation_rows(tmp_obs_path.base, tmp_obs_path.season, [row1])
    result = persistence.append_observation_rows(tmp_obs_path.base, tmp_obs_path.season, [row2])
    assert result.written == 1
    assert len(persistence.read_observation_rows(tmp_obs_path.sources)) == 2
...
def test_rows_are_immutable_on_disk_no_line_ever_rewritten(tmp_obs_path):
    ...
    persistence.append_observation_rows(tmp_obs_path.base, tmp_obs_path.season, [row])
    original_content = tmp_obs_path.text()

    # Appending a genuinely new row must never touch the first line.
    row2 = make_corpus_row(observation=make_observation(kalshi_market_ticker="MKT-2"))
    persistence.append_observation_rows(tmp_obs_path.base, tmp_obs_path.season, [row2])
    new_content = tmp_obs_path.text()
...
def test_new_process_reading_same_path_sees_prior_rows(tmp_obs_path):
    ...
    row = make_corpus_row()
    persistence.append_observation_rows(tmp_obs_path.base, tmp_obs_path.season, [row])
    keys = persistence.read_observation_keys(tmp_obs_path.sources)
    assert row.observation_key in keys


def test_reading_nonexistent_file_returns_empty_not_an_error(tmp_path):
    path = corpus_helpers.ref(tmp_path, persistence.OBSERVATIONS_SUBDIR, 2099)
    assert persistence.read_observation_rows(path.sources) == []
    assert persistence.read_observation_keys(path.sources) == set()
...
def test_settlement_identical_fact_deduped(tmp_path):
    ...
    r1 = persistence.append_settlement_rows(path.base, path.season, [settlement])
    r2 = persistence.append_settlement_rows(path.base, path.season, [settlement])
    assert r1.written == 1
    assert r2.written == 0
...
def test_settlement_state_transition_appends_new_row(tmp_path):
    ...
    persistence.append_settlement_rows(path.base, path.season, [settlement_pending])

    final = extract_game_result(
        {"status": "final", "homePoints": 31, "awayPoints": 24}, game_id="g1", season=2026, captured_at=NOW
    )
    settlement_final = settle_market(obs, final, settled_at=NOW)
    result = persistence.append_settlement_rows(path.base, path.season, [settlement_final])
...
def test_capture_state_same_state_reobserved_is_deduped(tmp_path):
    ...
    r1 = persistence.append_capture_state_rows(path.base, path.season, [rec])
    r2 = persistence.append_capture_state_rows(path.base, path.season, [rec])
    assert r1.written == 1
    assert r2.written == 0
...
def test_capture_state_transition_appends(tmp_path):
    ...
    assert result.written == 1
    rows = persistence.read_capture_state_rows(path.sources)
    assert len(rows) == 2
