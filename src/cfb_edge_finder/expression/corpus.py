...
def load_contract_snapshots(
    observations: Path | Iterable[Path] | None,
    *,
    ...
    `observations` is anything `research.shards.as_source_paths` accepts:
    a single legacy monolith, a shard directory, or the ordered list from
    `persistence.corpus_sources`. `ledger_load_count` still counts ONE
    corpus load -- the invariant is about re-deriving history once, not
    about how many files that history lives in."""
