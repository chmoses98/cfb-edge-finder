...
def _row(kind: str, game_id: str, source: str, status: int, raw: str, parsed: dict, now: datetime, extra: dict | None = None) -> dict:
    return {
        "schema_version": "live_sidecar_v1", "kind": kind, "game_id": str(game_id), "fetched_at": now.isoformat(),
        "source": source, "http_status": status, "payload_sha256": hashlib.sha256(raw.encode("utf-8", "replace")).hexdigest(),
        "parsed": parsed, **(extra or {}),
    }
...
def main() -> int:
    ...
    up = g[(g.kick > now) & (g.kick <= now + timedelta(days=args.horizon_days))].sort_values("kick").head(args.max_games)
    ...
    for r in up.itertuples(index=False):
        ...
        params = {"latitude": float(r.latitude), "longitude": float(r.longitude), "timezone": "UTC",
                  "hourly": "temperature_2m,relative_humidity_2m,precipitation,precipitation_probability,rain,snowfall,wind_speed_10m,wind_gusts_10m,wind_direction_10m,cloud_cover",
                  "forecast_days": 14, "wind_speed_unit": "kmh"}
        st, txt = _get(url, params)
        parsed: dict = {}
        if st == 200:
            try:
                body = json.loads(txt); h = pd.DataFrame(body["hourly"]); h["time"] = pd.to_datetime(h.time, utc=True)
                kh = r.kick.floor("h"); win = h[(h.time >= kh) & (h.time <= kh + pd.Timedelta(hours=3))]
                if len(win):
                    parsed = {c: (float(win.iloc[0][c]) if pd.notna(win.iloc[0][c]) else None) for c in win.columns if c != "time"}
                    parsed["wind_gusts_10m_max3h"] = float(win.wind_gusts_10m.max()); parsed["precipitation_sum3h"] = float(win.precipitation.sum())
            except Exception as exc:  # noqa: BLE001
                parsed = {"parse_error": f"{type(exc).__name__}: {exc}"}
        out["weather_forecast"].append(_row("weather_forecast", r.game_id, url, st, txt, parsed, now,
                                            {"kickoff_utc": r.kick.isoformat(), "lead_hours": round(lead_h, 2), "venue_id": int(r.venue_id) if pd.notna(r.venue_id) else None}))
        time.sleep(0.15)
        # --- ESPN odds ---------------------------------------------------------
        ...
        if st == 200:
            try:
                body = json.loads(txt); books = []
                for it in body.get("items", []):
                    books.append({"provider": (it.get("provider") or {}).get("name"), "details": it.get("details"), "spread": it.get("spread"),
                                  "over_under": it.get("overUnder"), "home_ml": ((it.get("homeTeamOdds") or {}).get("moneyLine")),
                ...
                parsed = {"parse_error": f"{type(exc).__name__}: {exc}"}
        out["espn_odds"].append(_row("espn_odds", r.game_id, url, st, txt, parsed, now, {"kickoff_utc": r.kick.isoformat(), "lead_hours": round(lead_h, 2)}))
        time.sleep(0.15)
    ...
    for r in up.itertuples(index=False):
        ...
        for c in comps:
            ...
            if st2 == 200:
                try:
                    body = json.loads(txt2); parsed = {"count": body.get("count"), "items": [i.get("$ref") for i in body.get("items", [])][:50]}
                except Exception as exc:  # noqa: BLE001
                    parsed = {"parse_error": f"{type(exc).__name__}: {exc}"}
            out["espn_injuries"].append(_row("espn_injuries", r.game_id, url, st2, txt2, parsed, now, {"espn_team_id": tid}))
            time.sleep(0.15)
    for kind, rows in out.items():
        p = args.out_dir / kind / f"{season}.jsonl"; p.parent.mkdir(parents=True, exist_ok=True)
